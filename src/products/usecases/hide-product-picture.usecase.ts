...
export class HideProductPictureUseCase {
    ...
    async execute(pictureId: string, user: User): Promise<void> {
        const picture = await this.productPictureRepository.findOne({ where: { id: pictureId } });

        if (!picture) {
            this.logger.warn(`Product picture with id ${pictureId} not found`);
            throw new NotFoundException(`Product picture with id ${pictureId} not found`);
        }

        if (!picture.isActive) {
            this.logger.warn(`Product picture with id ${pictureId} is already hidden`);
            throw new BadRequestException(`Product picture with id ${pictureId} is already hidden`);
        }
        ...
        this.logger.log(`Product picture with id ${pictureId} has been hidden by user ${user.id}`);
    }
}