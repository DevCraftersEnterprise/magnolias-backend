...
export class ChangeOrderStatusUseCase {
  ...
  async execute(
    ...
    if (order.status === OrderStatus.CANCELED) {
      ...
      throw new BadRequestException(
        `Cannot change status of a canceled order (ID: ${id})`,
      );
    }
    ...
    this.logger.log(
      `Order ID ${id} status changed to ${orderStatus} successfully`,
    );
  ...
  private async handleCancellation(
    ...
    this.logger.log(
      `Cancellation record created for order ID ${order.id} with reason: ${reason}`,
    );
  }
}
