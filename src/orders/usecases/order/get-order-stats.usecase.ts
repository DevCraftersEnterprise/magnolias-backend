...
export class GetOrderStatsUseCase {
  ...
  async execute(user: User, branchId?: string): Promise<OrderStatsResponse> {
    this.logger.log(
      `Getting order stats for user ${user.id} with role ${user.role} and branchId ${branchId}`,
    );
    ...
    if (user.role === UserRoles.ADMIN || user.role === UserRoles.SUPER) {
      ...
      if (!user.branch.id) {
        ...
        throw new BadRequestException(
          'User does not have an associated branch',
        );
      }
    ...
    this.logger.log(
      `Order stats fetched successfully for branch ${filterBranchId || 'all branches'}`,
    );
