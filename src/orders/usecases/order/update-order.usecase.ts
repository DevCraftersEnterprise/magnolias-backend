...
export class UpdateOrderUseCase {
  ...
  private async handleDeliveryAddress(
    ...
    if (!deliveryAddress) {
      this.logger.log(
        `No delivery address provided for order with ID: ${order.id}, skipping address update`,
      );
      return;
    }
    ...
    if (deliveryAddress.useCustomerAddress) {
      this.logger.log(
        `Using customer's address for order with ID: ${order.id}`,
      );

      if (!customer.address) {
        this.logger.error(
          `Customer with ID ${customer.id} does not have an address`,
        );
        return;
      }
      ...
      if (deliveryAddress.saveAsCommonAddress) {
        if (!deliveryAddress.commonAddressName) {
          this.logger.warn(
            'Common address name is required when saveAsCommonAddress is true',
          );
          return;
        }
  ...
  private async handleOrderDetails(
    ...
    const productsPromises = details.map((detail) =>
      this.productsService.findProductByTerm(detail.productId),
    );
    ...
    if (detailsToUpdate.length > 0)
      await this.orderDetailRepository.save(detailsToUpdate);
  ...
  private async handleOrderFlowers(
    ...
    const flowersPromises = flowers.map((flower) =>
    ...
    for (const flowerDto of flowers) {
      ...
      if (!flower) {
        this.logger.warn(
          `Flower with ID ${flowerDto.flowerId} not found, skipping this flower`,
        );
        continue;
      }

      const existingFlower = existingFlowers.find(
        (f) => f.flower.id === flowerDto.flowerId,
      );
    ...
    if (flowersToUpdate.length > 0)
      await this.orderFlowerRepository.save(flowersToUpdate);
    if (flowersToCreate.length > 0)
      await this.orderFlowerRepository.save(flowersToCreate);
  }
