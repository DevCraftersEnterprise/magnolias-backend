...
import { CreateOrderDeliveryAddressDto, NewAddressDataDto } from '../../dto/create-order-delivery-address.dto';
...
export class CreateOrderUseCase {
    ...
    async execute(createOrderDto: CreateOrderDto, user: User, referenceImages?: Express.Multer.File[]) {
        const { customerId, branchId, orderType, details, deliveryAddress, flowers, ...orderDto } = createOrderDto;
        ...
        if (deliveryAddress && (orderType === OrderType.DOMICILIO || orderType === OrderType.FLOR || orderType === OrderType.EVENTO)) {
    ...
    private async generateOrderCode(orderType: OrderType, branch: Branch): Promise<string> {
            ...
            if (deliveryAddress.newAddress) {
                ...
                if (deliveryAddress.saveAsCommonAddress) {
                    if (!deliveryAddress.commonAddressName) {
                        this.logger.warn('Common address name is required to save new address as common');
                        return;
                    }

                    const newCommonAddress = await this.addressesService.create({ name: deliveryAddress.commonAddressName, ...deliveryAddressDto }, user);
    ...
    private async handleFlowerForOrder(addFlowersToOrderDto: AddFlowerToOrderDto[], order: Order, user: User): Promise<void> {
        const orderFlowers: OrderFlower[] = [];

        const flowersPromises = addFlowersToOrderDto.map(flower => this.flowersService.findOne(flower.flowerId));
    ...
    private async calculateOrderTotal(details: CreateOrderDetailDto[], order: Order, user: User, referenceImages?: Express.Multer.File[]): Promise<void> {
        let totalAmount = 0;

        const productsPromises = details.map(product => this.productsService.findProductByTerm(product.productId));

        const products = await Promise.all(productsPromises);

        const orderDatails: OrderDetail[] = [];

        const folder = process.env.NODE_ENV === 'development' ? `dev/magnolias/orders/reference-images` : `magnolias/orders/reference-images`;
