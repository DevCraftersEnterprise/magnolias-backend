...
export class ResetPasswordForUserUseCase {
  ...
  async execute(
    ...
    this.logger.log(
      `Resetting password for user with username ${username} by user: ${currentUser.id}`,
    );
    ...
    if (!user) {
      this.logger.warn(
        `User with username ${username} not found for password reset`,
      );
    ...
    this.logger.log(
      `User with ID ${user.id} updated successfully by user ${currentUser.id}`,
    );
