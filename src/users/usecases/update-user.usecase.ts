...
export class UpdateUserUseCase {
    ...
    async execute(updateUserDto: UpdateUserDto, currentUser: User): Promise<Partial<User>> {
        ...
        if (user.role === UserRoles.BAKER && role !== UserRoles.BAKER) {
            this.logger.log(`User with ID ${id} is no longer a BAKER, clearing branches`);
            user.branches = [];
        }
        ...
        if (user.role !== role && role === UserRoles.BAKER) {
            ...
            Object.assign(user, updateUserDto, { updatedBy: currentUser, branches, branch: null });
        }
        ...
        if (branchId && role !== UserRoles.BAKER) {
            ...
            Object.assign(user, updateUserDto, { updatedBy: currentUser, branch, branches: [] });
        }
        ...
        this.logger.log(`User with ID ${id} updated successfully by user ${user.id}`);
