...
export class UpdateCommonAddressUseCase {
    ...
    async execute(id: string, updateCommonAddressDto: UpdateCommonAddressDto, user: User): Promise<CommonAddress> {
        const address = await this.commonAddressRepository.findOne({ where: { id } });
        ...
        const duplicateAddress = await this.checkForDuplicateAddressUtil.checkForDuplicate(street ?? address.street, number ?? address.number, neighborhood ?? address.neighborhood);

        if (duplicateAddress && duplicateAddress.id !== id) {
            this.logger.warn(`Duplicate address found with ID ${duplicateAddress.id} for update`);
            throw new ConflictException(`Another address with the same street, number, and neighborhood already exists`);
        }
