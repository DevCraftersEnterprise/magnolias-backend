...
export class CreateCommonAddressUseCase {
  ...
  async execute(
    createCommonAddressDto: CreateCommonAddressDto,
    user: User,
    ...
    if (duplicateAddress) {
      this.logger.warn(
        `Duplicate address found: ${street} ${number}, ${neighborhood}`,
      );
    ...
    this.logger.log(
      `Created new address: ${street} ${number}, ${neighborhood} by user ${user.id}`,
    );
