import { FindAllFruitsUseCase } from './find-all-fruits.usecase';

describe('FindAllFruitsUseCase', () => {
  let findAndCountMock: jest.Mock;
  let useCase: FindAllFruitsUseCase;

  beforeEach(() => {
    findAndCountMock = jest.fn();
    useCase = new FindAllFruitsUseCase({
      findAndCount: findAndCountMock,
    } as never);
  });

  it('retorna la respuesta paginada cuando se envían limit y offset', async () => {
    findAndCountMock.mockResolvedValue([[{ id: '1', name: 'FRESA' }], 1]);
