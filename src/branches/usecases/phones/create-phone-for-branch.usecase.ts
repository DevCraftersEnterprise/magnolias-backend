...
export class CreatePhoneForBranchUseCase {
    ...
    async execute(createPhoneDto: CreatePhonesDto, user: User, branchId: string): Promise<Phone> {
        ...
        this.logger.log(`Created phone with ID ${savedPhones.id} for branch ID ${branchId}`);

        Object.assign(branch, { phones: savedPhones, updatedBy: user });

        await this.branchRepository.save(branch);

        this.logger.log(`Updated branch with ID ${branchId} to associate new phone with ID ${savedPhones.id}`);
