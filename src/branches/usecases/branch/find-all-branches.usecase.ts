...
export class FindAllBranchesUseCase {
  ...
  async execute(
    branchesFilterDto: BranchesFilterDto,
    ...
    if (limit !== undefined && offset !== undefined) {
      this.logger.log(
        `Found ${total} branches matching filters. Returning page ${Math.floor(offset / limit) + 1} with ${branches.length} branches.`,
      );
    ...
    this.logger.log(
      `Found ${total} branches matching filters. Returning all results.`,
    );
