...
export class RemoveBranchUseCase {
    ...
    async execute(updateBranchDto: UpdateBranchDto, user: User): Promise<void> {
        ...
        if (!branch) {
            this.logger.warn(`Branch with ID ${id} not found for removal or already inactive`);
            throw new NotFoundException(`Branch with ID ${id} not found or already inactive`);
        }
        ...
        this.logger.log(`Branch with ID ${id} removed successfully by user ${user.id}`);
    }
}